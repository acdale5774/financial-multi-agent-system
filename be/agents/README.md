...
3. **Enforce (deterministic).** Markers that don't resolve to the ledger
   trigger one repair turn (the corrective message lists the real inventory);
   whatever is still invalid is stripped and reported in
   `validation.invalid_markers_stripped`. The endpoint never 500s over
   citations — it degrades honestly.
4. **Lint (warning severity, by design).** Two heuristics are reported but
   never block: sentences stating money/percent/large figures without a
   marker, and marked sentences whose numbers match no cited value under
   raw/K/M/B/T/percent scalings (0.5 % tolerance for display rounding).
   They are warnings because sentence splitting and number matching are
   heuristics — hard-failing them would strip true claims (e.g. "3.0× higher",
   a derived comparison of two cited figures).
...
evidence is structurally impossible (the residual transcription risk lives in
prose only, where the value-mismatch lint watches it). Rendering is a pure
function (`tools/chart_tool.py`, Agg backend, `Figure` directly — pyplot's
global state is not thread-safe under concurrent requests). The PNG is served
at `/charts/…` for curl users today; the declarative spec rides along in
`charts[].spec` for the future frontend to re-render natively.

### How the five example questions flow

| Question | Route | What happens |
|---|---|---|
| Collate Revenue/Net Income/EPS for X, last 4 quarters + plot EPS | quant | one pivot query projecting citation keys (EPS computed as `net_income / NULLIF(shares_diluted,0)`); markdown table, one marker per figure; `create_chart` plots the cited EPS ids |
...
- A valid citation id can sit on a subtly wrong sentence; the value-mismatch
  lint narrows but does not close this (see tradeoffs).
...
- Chart references `[C#]`/image markdown are not validated like S/D markers.
